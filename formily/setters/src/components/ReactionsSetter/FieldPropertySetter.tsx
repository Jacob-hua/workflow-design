...
export const FieldPropertySetter: React.FC<IFieldPropertySetterProps> = (
  ...
  return (
    <div className={prefix}>
      ...
      <div className={prefix + '-attr-wrapper'}>
        <Select
          defaultValue={selectKeys[0]}
          style={{
            width: 300
          }}
          options={(FieldProperties.map((key) => {
            if (isPlainObj(key)) {
              return {
                value: key.key,
                label: GlobalRegistry.getDesignerMessage(`SettingComponents.ReactionsSetter.${key.token || key.key
                  }`)
              }
            }
            return {
              value: key,
              label: GlobalRegistry.getDesignerMessage(`SettingComponents.ReactionsSetter.${key}`)
            }
          }))}
